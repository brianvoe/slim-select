...
function m(e, t = f) {
	return p(getComputedStyle(e).getPropertyValue("--ss-animation-timing").trim(), t);
}
function h() {
...
function _(e, t) {
	if (h() || t.duration <= 0 || typeof e.animate != "function") return g(t);
	let n = e.animate([{
		transform: "scale(1)",
		opacity: 1
	}, {
...
var y = class {
	...
	options;
	constructor(e) {
		if (this.id = !e.id || e.id === "" ? a() : e.id, this.label = e.label || "", this.selectAll = e.selectAll === void 0 ? !1 : e.selectAll, this.selectAllText = e.selectAllText || "Select All", this.closable = e.closable || "off", this.options = [], e.options) for (let t of e.options) this.options.push(new y(t));
	}
}, x = class {
	selectType = "single";
	data = [];
	...
	validateDataArray(e) {
		...
		for (let t of e) if (t) if (t instanceof b || "label" in t) {
		...
		} else if (t instanceof y || "text" in t) {
	...
	partialToFullData(e) {
		...
		return e.forEach((e) => {
			if (e) {
				if (e instanceof b || "label" in e) {
					let n = [];
					"options" in e && e.options && e.options.forEach((e) => {
						n.push(new y(e));
					}), n.length > 0 && t.push(new b(e));
				}
				(e instanceof y || "text" in e) && t.push(new y(e));
			}
		}), t;
	}
	setData(e, t = !1) {
	...
	addOption(e, t = !1) {
		...
		} else this.setData(this.getData().concat(new y(e)));
	}
	setSelectedBy(e, t, n = !1) {
		let r = null, i = !1, a = [];
		for (let n of this.data) {
			if (n instanceof b) for (let o of n.options) {
				r ||= o;
				let n = o[e] || "";
				o.selected = i ? !1 : t.includes(n), o.selected && (a.push(o), this.selectType === "single" && (i = !0));
			}
			n instanceof y && (r ||= n, n.selected = i ? !1 : t.includes(n[e]), n.selected && (a.push(n), this.selectType === "single" && (i = !0)));
		}
	...
	getOptgroupByID(e) {
		for (let t of this.data) if (t instanceof b && t.id === e) return t;
		return null;
	}
	getOptionByID(e) {
	...
	getFirstOption() {
		let e = null;
		for (let t of this.data) if (t instanceof b ? e = t.options[0] : t instanceof y && (e = t), e) break;
		return e;
	}
	search(e, t) {
		...
		return this.data.forEach((r) => {
			if (r instanceof b) {
				...
				if (r.options.forEach((r) => {
					(!e || e(r)) && (t ? i.push(new y(r)) : n.push(new y(r)));
				}), i.length > 0) {
					let e = new b(r);
					e.options = i, n.push(e);
				}
			}
			r instanceof y && (!e || e(r)) && n.push(new y(r));
		}), n;
	}
	...
	waitForAnimation(e) {
		let t = m(this.content.main);
		return v(this.content.main, void 0, t);
	}
	close() {
	...
	renderMultipleValues() {
		...
		let r = m(this.main.main);
		for (let e of n) this.addClasses(e, this.classes.valueOut), _(e, {
			duration: r,
			onFinish: () => {
		...
		if (this.addClasses(n, this.classes.valueText), n.textContent = e.text, t.appendChild(n), !e.mandatory) {
			...
			this.addClasses(n, this.classes.valueDelete), n.setAttribute("tabindex", "0"), n.setAttribute("role", "button"), n.setAttribute("aria-label", `${this.settings.removeText} ${e.text}`), n.onclick = (t) => {
				...
				if (!(this.settings.minSelected && i.length < this.settings.minSelected) && (this.callbacks.beforeChange && (n = this.callbacks.beforeChange(i, r) === !0), n)) {
					let e = [];
					for (let t of i) {
						if (t instanceof b) for (let n of t.options) n.id && e.push(n.id);
						t instanceof y && e.push(t.id);
					}
	...
	renderOptions(e) {
		if (this.lastRenderedOptions = e.map((e) => e instanceof y ? [e] : e.options.map((e) => new y(e))).flat(), this.content.list.innerHTML = "", e.length === 0) {
			let e = document.createElement("div");
		...
		this.settings.allowDeselect && !this.settings.isMultiple && (this.store.filter((e) => e.placeholder, !1).length || this.store.addOption(new y({
			text: "",
			value: "",
			selected: !1,
			placeholder: !0
		...
		for (let n of e) {
			if (n instanceof b) {
				...
				if (this.addClasses(a, this.classes.optgroupActions), r.appendChild(a), this.settings.isMultiple && n.selectAll) {
					...
					s.setAttribute("d", this.classes.optgroupSelectAllCheck), i.appendChild(s), e.addEventListener("click", (e) => {
						...
						if (t) {
							...
							for (let e of n.options) e.id && !this.store.getOptionByID(e.id) && this.callbacks.addOption(new y(e));
				...
				e.appendChild(r);
				for (let r of n.options) e.appendChild(this.option(new y(r))), t.appendChild(e);
			}
			n instanceof y && t.appendChild(this.option(n));
		}
...
var w = class {
	...
	observeCall(e) {
		...
		let { classChanged: t, disabledChanged: n, optgroupOptionChanged: r, selectionChanged: i } = C(e, this.select), a = i;
	...
	updateOptions(e) {
		if (!(!e || e.length === 0)) {
			...
			for (let t of e) t instanceof b && this.select.appendChild(this.createOptgroup(t)), t instanceof y && this.select.appendChild(this.createOption(t));
...
var k = class {
	...
	applyStructure(e, t, n = !1) {
		...
		if (D(r, e)) return;
		let s = r.getSelected(), l = r.validateDataArray(e);
		if (l) {
			o.error ? o.error(l) : this.deps.onError && this.deps.onError(l);
			return;
		}
		r.setData(e, n);
		let u = r.getData();
		i.updateOptions(u), a.renderValues(), a.renderOptions(u), o.afterChange && !c(s, r.getSelected()) && o.afterChange(r.getSelectedOptions());
	}
	applySelection(e, t, n) {
		let { store: r, select: i, render: a, events: o } = this.deps, s = r.getSelected(), l = O(r, e);
		if (E(s, l)) return;
		r.setSelectedBy("id", l), i.setSelectedByValue(r.getSelectedValues()), a.renderValues();
		let u = a.content.search.input.value;
		u === "" ? a.renderOptions(r.getData()) : t === "ui" && this.deps.search ? this.deps.search(u) : a.renderOptions(r.getData()), n && o.afterChange && !c(s, r.getSelected()) && o.afterChange(r.getSelectedOptions());
	}
	applyAddOption(e) {
		...
		let o = t.getData();
		n.updateOptions(o), r.renderValues(), r.renderOptions(o), i.afterChange && !c(a, t.getSelected()) && i.afterChange(t.getSelectedOptions());
	}
}, A = class {
	selectEl;
	settings;
	...
	constructor(e) {
		...
		this.selectEl.dataset.ssid && this.destroy(), this.settings = new T(e.settings), this.cssClasses = new i(e.cssClasses);
		let t = [
			"beforeOpen",
			"afterOpen",
			"beforeClose",
		...
		this.store = new x(this.settings.isMultiple ? "multiple" : "single", n), e.data && this.select.updateOptions(this.store.getData());
		let r = {
		...
		this.render = new S(this.settings, this.cssClasses, this.store, r), this.sync = new k({
			select: this.select,
			store: this.store,
			render: this.render,
			events: this.events,
	...
	mounted() {
		...
		}, this.slim = new A(e), this.syncModelValueToSlimSelect(!1);
	},
	updated() {
...
function P(t, i, a, o, s, c) {
	return n(), e("select", {
		multiple: t.multiple,
		ref: "slim"
	}, [r(t.$slots, "default")], 8, N);
}
//#endregion
//#region src/slim-select/vue/index.ts
var F = /* @__PURE__ */ M(j, [["render", P]]);
//#endregion
export { b as Optgroup, y as Option, T as Settings, F as default };
